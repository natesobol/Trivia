...
Private Sub ExecuteQuery2 (con As SQL, in As InputStream,  resp As ServletResponse) As String
	...
	Dim cmd1 As DBCommand = m.Get("command")
	Dim limit As Int = m.Get("limit")
	Dim rs As ResultSet = con.ExecQuery2(Main.rdcConnector1.GetCommand(cmd1.Name), cmd1.Parameters)
...
Private Sub ExecuteBatch2(con As SQL, in As InputStream, resp As ServletResponse) As String
	...
	Try
		...
		For Each cmd1 As DBCommand In commands
			con.ExecNonQuery2(Main.rdcConnector1.GetCommand(cmd1.Name), _
				cmd1.Parameters)
		Next
